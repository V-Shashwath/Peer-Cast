...
# PeerCast VPS Setup Script
# This script helps set up PeerCast on a fresh Ubuntu/Debian VPS

# Exit on error
set -e

echo "=== PeerCast VPS Setup Script ==="
echo "This script will install Java, Node.js, Nginx, and set up PeerCast."
...
# Create the peercast configuration file with the correct content
echo "Creating /etc/nginx/sites-available/peercast..."
cat <<EOF | sudo tee /etc/nginx/sites-available/peercast
server {
    listen 80;
...
# Create the symbolic link to enable the peercast site
sudo ln -sf /etc/nginx/sites-available/peercast /etc/nginx/sites-enabled/peercast
...
if [ $? -eq 0 ]; then
    ...
    echo "Nginx configuration test failed. Please check /etc/nginx/nginx.conf and /etc/nginx/sites-available/peercast."
    exit 1
fi
